...
Когда multiple горутин одновременно вызывают Do() в первый раз:
...
4. **После освобождения мьютекса** планировщик пробуждает одну из ожидающих горутин

### **Пробуждение горутин**

При разблокировке мьютекса:
- **Одна горутина** пробуждается и проверяет `o.done`
- **Поскольку done=1**, горутина немедленно возвращается из Do()
- **Остальные горутины** остаются заблокированными до следующего пробуждения
