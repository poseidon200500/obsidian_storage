...
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Worker Pool**    | Когда нужно ограничить количество одновременно выполняемых задач для контроля над ресурсами (например, лимит подключений к БД, ограничение по памяти) |
| **Fan-out/Fan-in** | Когда нужно распараллелить обработку независимых данных (например, обработать 1000 файлов, выполнить множество HTTP запросов)                         |
| **Pipeline**       | Когда данные проходят через последовательные этапы обработки (например, чтение → фильтрация → агрегация → запись)                                     |
| **Pub/Sub**        | Когда нужна слабосвязанная архитектура с множеством подписчиков на события (например, event-driven системы, нотификации)                              |
| **Barrier**        | Когда нужно дождаться завершения нескольких независимых операций перед продолжением работы (например, parallel запросы к API)                         |
